...
export const StudentSettings = () => {
  ...
  if (loading) return <div className="p-12 text-center text-slate-400 font-black uppercase animate-pulse tracking-widest">Accessing secure node...</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-12">
      <header>
        <h2 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Student Identity Matrix</h2>
        <p className="text-sm text-slate-500 font-medium">Manage your digital footprint and secure node settings</p>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1 space-y-6">
          <div className="bg-white rounded-[2rem] border-2 border-slate-50 p-8 shadow-sm text-center">
            <div className="w-24 h-24 bg-slate-900 rounded-[2rem] flex items-center justify-center mx-auto mb-6 text-white shadow-xl shadow-slate-200">
              <User size={48} />
            </div>
          ...
          <div className="bg-slate-900 rounded-[2rem] p-8 text-white">
            <h5 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-6 flex items-center">
              <Shield size={14} className="mr-2" /> Security Telemetry
            </h5>
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] font-black text-slate-200 uppercase font-mono">ENCRYPTED</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          <form onSubmit={handleUpdate} className="bg-white rounded-[2rem] border-2 border-slate-50 shadow-sm overflow-hidden">
            <div className="px-8 py-6 border-b border-slate-50 bg-slate-50/30">
              <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest flex items-center">
                <Settings size={16} className="mr-2 text-indigo-600" /> Identity Synchronization
              </h4>
            </div>
            <div className="p-8 space-y-8">
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center ml-1">
                    <User size={12} className="mr-2" /> Global Identity Name
                  </label>
                  <input 
                    type="text" 
                    value={fullName}
                    onChange={(e) => setFullName(e.target.value)}
                    className="w-full p-4 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-indigo-500 focus:bg-white outline-none font-bold text-sm transition-all"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center ml-1">
                    <Mail size={12} className="mr-2" /> Registry Email Address
                  </label>
                  <input 
                    type="email" 
                    value={email}
              ...
              <div className="pt-6 flex items-center justify-between border-t border-slate-50">
                ...
                >
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest">Refresh Registry</span>
                </button>
                <button 
                  type="submit"
                  disabled={updating}
                  className="flex items-center space-x-2 px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 disabled:opacity-50"
                >
          ...
          <div className="mt-8 p-8 bg-slate-50 border-2 border-slate-100 rounded-[2rem] flex items-center justify-between group cursor-pointer hover:border-indigo-100 transition-all">
            <div>
               <h5 className="text-xs font-black text-slate-900 uppercase tracking-tight flex items-center mb-1">
                 <Shield size={16} className="mr-2 text-slate-400 group-hover:text-indigo-600 transition-colors" /> Node Security Protocol
               </h5>
               <p className="text-[11px] text-slate-500 font-medium">Update your secure access credentials and multi-factor tokens.</p>
            </div>
