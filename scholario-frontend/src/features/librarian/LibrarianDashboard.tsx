...
export const LibrarianDashboard = () => {
  ...
  const activeIssuesCount = data?.getDueDates.filter(i => i.state.type !== 'RETURNED').length || 0;
  const overdueCount = data?.getDueDates.filter(i => i.state.type === 'OVERDUE').length || 0;
  
  const today = new Date().toISOString().split('T')[0];
  const returnedTodayCount = data?.getDueDates.filter(i => {
    if (i.state.type !== 'RETURNED' || !i.returnDate) return false;
    return i.returnDate.startsWith(today) || new Date(i.returnDate).toISOString().split('T')[0] === today;
  }).length || 0;
  
  const reservationsCount = 0; // Global reservations query not available
  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          title="Active Issues" value={activeIssuesCount.toString()} delay="100ms" color="text-indigo-600 bg-indigo-600"
          icon={<BookOpen size={24} />} 
        />
        <StatCard 
          title="Overdue" value={overdueCount.toString()} delay="200ms" color="text-rose-600 bg-rose-600"
          icon={<AlertCircle size={24} />} 
        />
        <StatCard 
          title="Returned (Today)" value={returnedTodayCount.toString()} delay="300ms" color="text-emerald-600 bg-emerald-600"
          icon={<CheckCircle size={24} />} 
        />
        <StatCard 
          title="Reservations" value={reservationsCount.toString()} delay="400ms" color="text-amber-600 bg-amber-600"
          icon={<Clock size={24} />} 
        />
      </div>
                    ...
                    <tr key={issue.id} className="hover:bg-gray-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="font-semibold text-gray-900">
                          {booksData?.getAllBooks.find(b => b.id === issue.bookId)?.title || `Book #${issue.bookId}`}
                        </div>
                        <div className="text-xs text-gray-400">ID: {issue.id.substring(0, 8)}...</div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-600">
                        {studentsData?.getStudentList.find(s => s.id === issue.userId)?.fullName || `User #${issue.userId}`}
                      </td>
                      <td className="px-6 py-4">
