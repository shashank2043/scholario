...
const validationSchema = yup.object({
  fullName: yup.string().required('Global identity name is required'),
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <Card sx={{ bgcolor: (theme) => theme.palette.mode === 'dark' ? 'rgba(129, 140, 248, 0.1)' : '#111827', color: (theme) => theme.palette.mode === 'dark' ? 'text.primary' : '#fff', p: 3, borderRadius: 3, border: 'none' }}>
              <Typography variant="caption" sx={{ fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'text.secondary', display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                <ShieldIcon sx={{ fontSize: 16 }} /> Security Telemetry
              </Typography>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                <Box sx={{ display: 'flex', justifyContext: 'space-between', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase' }}>Registry Status</Typography>
                  <Typography variant="caption" sx={{ fontWeight: 900, color: '#10b981', textTransform: 'uppercase' }}>Verified</Typography>
                </Box>
                <Box sx={{ display: 'flex', justifyContext: 'space-between', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase' }}>Access Token</Typography>
                  <Typography variant="caption" sx={{ fontWeight: 900, color: '#f3f4f6', fontFamily: 'monospace' }}>ENCRYPTED</Typography>
                </Box>
              </Box>
            </Card>
          </Box>
            ...
            <CardContent sx={{ p: 4, display: 'flex', flexDirection: 'column', gap: 3 }}>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                <Typography variant="caption" sx={{ fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'text.secondary', display: 'flex', alignItems: 'center', gap: 1 }}>
                  <UserIcon sx={{ fontSize: 14 }} /> Global Identity Name
                </Typography>
                <TextField 
                  fullWidth
                  id="fullName"
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                <Typography variant="caption" sx={{ fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'text.secondary', display: 'flex', alignItems: 'center', gap: 1 }}>
                  <MailIcon sx={{ fontSize: 14 }} /> Registry Email Address
                </Typography>
                <TextField 
                  fullWidth
                  id="email"
          ...
          <Card sx={{ mt: 3,  borderRadius: 3, p: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box>
              <Typography sx={{ fontWeight: 800, fontSize: '13px', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 1, color: 'text.primary' }}>
                <ShieldIcon sx={{ fontSize: 16, color: 'text.secondary' }} /> Node Security Protocol
              </Typography>
            ...
            <Button 
              variant="outlined" 
              onClick={() => {
                const authUrl = import.meta.env.VITE_KEYCLOAK_URL || 'http://localhost:8080/';
                const realm = import.meta.env.VITE_KEYCLOAK_REALM || 'scholario';
                const base = authUrl.endsWith('/') ? authUrl : `${authUrl}/`;
                const signingInUrl = `${base}realms/${realm}/account/account-security/signing-in`;
                window.open(signingInUrl, '_blank');
              }}
              sx={{ 
                fontSize: '10px', 
                fontWeight: 800, 
